...
for /f "tokens=2 delims=:," %%a in ('findstr /i "%CSM_NAME%" "%~dp0step1-formatting\configs\ars_config.json"') do (
    set CURRENT_PATH=%%~a
)
...
if /i "%CLIENT_ID%"=="ALL" (
    ...
    echo ======================================================================
    echo.
    python "%~dp0run-step1.py" --csm %CSM_NAME%
) else (
    echo   Running for %CSM_NAME% -- Client %CLIENT_ID%...
    echo ======================================================================
    echo.
    python "%~dp0run-step1.py" --csm %CSM_NAME% --client %CLIENT_ID%
)
