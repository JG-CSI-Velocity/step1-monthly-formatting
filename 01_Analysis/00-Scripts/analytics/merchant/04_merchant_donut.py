# ===========================================================================
# MERCHANT DONUT: Top 8 + Other with Stats Panel (Conference Edition)
# ===========================================================================
# Donut chart + stats table side by side. (14,7).

top8_m = merch_agg.head(8).copy()
other_txns = total_txns_merch - top8_m['txn_count'].sum()
other_accts = total_accts_merch  # can't sum unique accounts for "other"

labels = top8_m['merchant_consolidated'].str[:25].tolist() + ['Other']
sizes = top8_m['txn_count'].tolist() + [other_txns]
...
ax1.text(0, 0, f"{total_txns_merch:,.0f}\nTransactions",
...
ax2.text(6.8, y, f"{other_txns:,.0f}", fontsize=11, fontweight='bold',
         color=GEN_COLORS['muted'], va='center', ha='right')
other_pct = other_txns / total_txns_merch * 100
ax2.text(9.5, y, f"{other_pct:.1f}%", fontsize=11, fontweight='bold',
         color=GEN_COLORS['muted'], va='center', ha='right')
