# ===========================================================================
# BUSINESS VS PORTFOLIO: Segment Merchant Comparison (Conference Edition)
# ===========================================================================
# Two-panel: business top 10 share vs same merchants' full portfolio share. (14,7).

if 'biz_agg' not in dir() or len(biz_agg) == 0:
    print("    No business data available. Skipping portfolio comparison.")
else:
    try:
        seg_top10_b = biz_agg.head(10)[['merchant_consolidated', 'txn_pct']].copy()
        seg_top10_b = seg_top10_b.sort_values('txn_pct', ascending=True)

        # Look up same merchants in the overall portfolio
        portfolio_share_b = merch_agg.set_index('merchant_consolidated')['txn_pct']
        seg_top10_b['portfolio_pct'] = seg_top10_b['merchant_consolidated'].map(portfolio_share_b).fillna(0)

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 7))

        names = seg_top10_b['merchant_consolidated'].str[:22].tolist()
        y_pos = range(len(seg_top10_b))

        # --- LEFT: Business share ---
        ax1.barh(y_pos, seg_top10_b['txn_pct'],
        ...
        ax1.set_title("Business Account Share", fontsize=20, fontweight='bold',
                       color=GEN_COLORS['primary'], pad=12)

        for j, (_, row) in enumerate(seg_top10_b.iterrows()):
        ...
        ax2.barh(y_pos, seg_top10_b['portfolio_pct'],
