...
# Klarna, Afterpay, etc.) by threat-like metrics: spend, account reach,
# transactions, $/account/month.
#
# BNPL is the remaining "non-bank" threat once wallets + P2P are stripped
# out. It matters because BNPL balances displace revolving credit and
...
if not _BOOT_OK:
    ...
    if len(bnpl) == 0:
        ...
        # --- n_months for per-month-per-account normalization -------------
        if 'DATASET_MONTHS' in globals() and DATASET_MONTHS:
        ...
        fig0.text(0.5, 0.97, CORE_SCOPE_NOTE,
